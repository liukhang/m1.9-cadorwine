...
//        }
    ...
    public function addActionLayoutHandles()
    {
        ...
        $update->addHandle(
            'firecheckout_' . Mage::getStoreConfig('firecheckout/general/layout')
        );
...
//            }
            ...
            if (!isset($result['redirect']) && !isset($result['error'])) {
                ...
                // Heidelpay
                if ($paymentData['method'] === 'hcdcc' || $paymentData['method'] === 'hcddc') {
                    $this->saveBillingAction();
                    $urls = $quote->getPayment()->getMethodInstance()->getHeidelpayUrl();
                    $redirectUrl = $urls['FRONTEND_REDIRECT_URL'];
                    $result['success'] = true;
                    $result['error']   = false;
                } else {
